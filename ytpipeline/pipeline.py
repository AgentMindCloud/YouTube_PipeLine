...
def execute(cfg, idea_text=None, count=1, auto=False, no_upload=False, force=False,
    ...
    if run_id:
        ...
        if idea_text:
            ...
            ideas = idea_mod.generate_ideas(cfg, count=max(count, 3), log=log)
            idea_dict = ideas[0]
            for alt in ideas[1:count + 2]:
    ...
    # ── 3. VOICE ─────────────────────────────────────────────────────────────
    log("▶ Stage 3/8 · VOICE")
    voice, durations, edge_words, tempo = tts_mod.synthesize(cfg, script["beats"], run, log=log)
    ...
    log(f"  · narration {sum(durations):.1f}s, {len(words)} timed words"
        + (" (exact boundaries)" if edge_words else " (estimated timing)"))
...
def approve(cfg, run_id, privacy=None, log=print):
    ...
    if not meta:
        raise SystemExit(f"{run_id} has no metadata.json — is it rendered? Run: python run.py run --run-id {run_id}")
...
from pathlib import Path  # noqa: E402  (used by upload_file)
